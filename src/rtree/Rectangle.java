...
{
    private Point dataCenter;
    private float r;
    private Point low; // 左下角的点
    private Point high; // 右上角的点

    public Rectangle(Point dataCenter,float r){
        this.dataCenter = dataCenter;
        this.r = r;
        float[] low = new float[dataCenter.getDimension()];
        float[] high = new float[dataCenter.getDimension()];
        for (int i=0;i<dataCenter.getDimension();i++){
            low[i] = dataCenter.getFloatCoordinate(i)-r;
            high[i] = dataCenter.getFloatCoordinate(i)+r;
        }
    ...
    public Point getDataCenter() {
        return dataCenter;
    }

    public void setDataCenter(Point dataCenter) {
        this.dataCenter = dataCenter;
    }
