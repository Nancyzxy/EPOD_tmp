...
public class RTDataNode extends RTNode {
    ...
    @Override
    protected ArrayList<Point> search(Rectangle rectangle) {
        ArrayList<Point> result = new ArrayList<>();
        //对于一个限制的rectangle来说，datacenter就是要搜索的点，r就是半径
        //对于一个data node来说，high = low = data
        for (int i = 0; i < usedSpace; i++) {
            if (rectangle.isIntersection(datas[i])) {
                //further check
                EuclideanDistance euclideanDistance = new EuclideanDistance();
                double[] d1 = new double[rectangle.getDataCenter().getData().length];
                for (int j=0;j<d1.length;j++){
                    d1[j] = rectangle.getDataCenter().getFloatCoordinate(j);
                }

                double[] d2 = new double[datas[i].getHigh().getData().length];
                for (int j=0;j<d2.length;j++){
                    d1[j] = datas[i].getHigh().getFloatCoordinate(j);
                }

                double x = euclideanDistance.distance(new Vector(d1),new Vector(d2));
                if (x<rectangle.getR()){
                    result.add(datas[i].getHigh());
                }
            }
        }
        return result;
